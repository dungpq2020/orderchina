...
interface ChinaWarehouseExportPageProps {
...
export default function ChinaWarehouseExportPage({ adminApiBaseUrl, loginUrl }: ChinaWarehouseExportPageProps) {
  ...
  async function saveRow(row: ScanRow, accessToken: string) {
    if (row.saving || row.data.status !== ARRIVED_CHINA_WAREHOUSE_STATUS) return;
    ...
    try {
      const res = await fetch(`${adminApiBaseUrl}/main-orders/tracking-codes/export-china-warehouse`, {
        method: "POST",
      ...
      if (!res.ok) {
        const body = await res.json().catch(() => null);
        updateRow(row.key, { saving: false, saveError: body?.error ?? `Xuất kho thất bại (status ${res.status}).` });
        return;
      }
  ...
  if (state.status === "error") {
    return (
      <AdminLayout title="Xuất kho Trung Quốc" adminApiBaseUrl={adminApiBaseUrl} accessToken="" onLogout={logout}>
  ...
  return (
    <AdminLayout title="Xuất kho Trung Quốc" adminApiBaseUrl={adminApiBaseUrl} accessToken={accessToken} onLogout={logout}>
      <h1 className="mb-1 text-xl font-semibold text-zinc-900">Xuất kho Trung Quốc</h1>
      <p className="mb-4 text-sm text-zinc-500">
        Quét mã vận đơn đã kiểm hàng kho Trung Quốc (hoặc gõ tay rồi nhấn Enter) — có thể sửa lại cân/kích thước lần cuối
        trước khi bấm Xuất kho để xác nhận kiện đã rời kho Trung Quốc.
      </p>
      ...
      {rows.length > 0 && (
        <div className="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-zinc-200 bg-orange-400 px-4 py-3 text-white">
            <h2 className="font-semibold">Đang xuất kho ({rows.length})</h2>
