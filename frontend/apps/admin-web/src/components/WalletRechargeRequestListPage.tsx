...
interface WalletRechargeRequestListPageProps {
  adminApiBaseUrl: string;
  loginUrl: string;
}

interface WalletRechargeRequestListItem {
  id: string;
  userId: string;
  username: string;
...
interface WalletRechargeRequestListResult {
  items: WalletRechargeRequestListItem[];
  totalCount: number;
  page: number;
  pageSize: number;
}
...
export default function WalletRechargeRequestListPage({ adminApiBaseUrl, loginUrl }: WalletRechargeRequestListPageProps) {
  ...
  const fetchRequests = useCallback(
    async (targetPage: number, accessToken: string): Promise<WalletRechargeRequestListResult> => {
      const res = await fetch(`${adminApiBaseUrl}/wallet-recharge/list?page=${targetPage}&pageSize=${PAGE_SIZE}`, {
      ...
      return (await res.json()) as WalletRechargeRequestListResult;
    },
    [adminApiBaseUrl],
  );

  const { state, page, goToPage, logout } = useAuthenticatedList<WalletRechargeRequestListResult>({
    adminApiBaseUrl,
    loginUrl,
  ...
  async function handleApprove(id: string, accessToken: string) {
    if (!window.confirm("Duyệt yêu cầu nạp này? Tiền sẽ được cộng vào ví khách hàng.")) return;

    setApprovingId(id);
    try {
      const res = await fetch(`${adminApiBaseUrl}/wallet-recharge/${id}/approve`, {
        method: "POST",
        headers: { Authorization: `Bearer ${accessToken}` },
      });
      ...
      goToPage(page);
      setToast("Đã duyệt — tiền đã được cộng vào ví.");
    } catch {
  ...
  if (state.status === "error") {
    return (
      <AdminLayout title="Yêu cầu nạp ví" adminApiBaseUrl={adminApiBaseUrl} accessToken="" onLogout={logout}>
  ...
  return (
    <AdminLayout title="Yêu cầu nạp ví" adminApiBaseUrl={adminApiBaseUrl} accessToken={accessToken} onLogout={logout}>
      <h1 className="mb-6 text-xl font-semibold text-zinc-900">Yêu cầu nạp ví ({data.totalCount})</h1>
        ...
        <table className="w-full text-left text-sm">
          <thead className="border-b border-zinc-200 bg-orange-400 text-white font-semibold">
            <tr>
              ...
              <th className="px-4 py-3 font-medium">Số tiền nạp (VNĐ)</th>
              <th className="px-4 py-3 font-medium">Nội dung</th>
              <th className="px-4 py-3 font-medium">Trạng thái</th>
              <th className="px-4 py-3 font-medium">Ngày nạp</th>
              ...
              return (
                <tr key={r.id} className="border-b border-zinc-100 last:border-0 align-top">
                  ...
                  <td className="px-4 py-3">
                    <div className="font-medium text-zinc-900">{formatMoney(r.amount)}</div>
                    <div className="mt-1 text-xs text-zinc-500">
                      Ngân hàng: {r.bankName ?? "Nạp trực tiếp"}
                    </div>
                  </td>
