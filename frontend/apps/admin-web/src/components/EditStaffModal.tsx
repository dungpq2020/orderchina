...
import type { StaffListItem } from "./StaffListPage";
...
interface EditStaffModalProps {
  ...
  accessToken: string;
  onClose: () => void;
  onSaved: (updated: Partial<StaffListItem> & { id: string }) => void;
}

export default function EditStaffModal({ staff, adminApiBaseUrl, accessToken, onClose, onSaved }: EditStaffModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`${adminApiBaseUrl}/staff/${staff.id}`, {
        method: "PUT",
        headers: {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4">
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl bg-white p-6 shadow-xl">
        <h2 className="mb-4 text-lg font-semibold text-zinc-900">Cập nhật thông tin nhân viên</h2>
