...
interface VietnamWarehouseCheckInPageProps {
...
export default function VietnamWarehouseCheckInPage({ adminApiBaseUrl, loginUrl }: VietnamWarehouseCheckInPageProps) {
  ...
  async function saveRow(row: ScanRow, accessToken: string) {
    if (row.saving || row.data.status === DELIVERED_TO_CUSTOMER_STATUS) return;
    ...
    try {
      const res = await fetch(`${adminApiBaseUrl}/main-orders/tracking-codes/check-in-vietnam-warehouse`, {
        method: "POST",
  ...
  if (state.status === "error") {
    return (
      <AdminLayout title="Kiểm kho Việt Nam" adminApiBaseUrl={adminApiBaseUrl} accessToken="" onLogout={logout}>
  ...
  return (
    <AdminLayout title="Kiểm kho Việt Nam" adminApiBaseUrl={adminApiBaseUrl} accessToken={accessToken} onLogout={logout}>
      <h1 className="mb-1 text-xl font-semibold text-zinc-900">Kiểm kho Việt Nam</h1>
      <p className="mb-4 text-sm text-zinc-500">
        Quét mã vận đơn (hoặc gõ tay rồi nhấn Enter) — nhập/sửa lại cân/kích thước khi kiện về tới kho Việt Nam, rồi bấm Cập
        nhật để xác nhận. Chỉ chặn mã đã giao khách.
      </p>
      ...
      {rows.length > 0 && (
        <div className="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-zinc-200 bg-orange-400 px-4 py-3 text-white">
            <h2 className="font-semibold">Đang kiểm kho ({rows.length})</h2>
